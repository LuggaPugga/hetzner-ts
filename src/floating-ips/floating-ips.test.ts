...
test("create floating IP throws error if token is invalid", async () => {
	const result = await invalidHetzner.floatingIps.create({
		type: "ipv4",
		home_location: "fsn1",
	});
	expect(result.success).toBe(false);
...
test("create floating IP creates a new IP and returns it", async () => {
	const params = {
		type: "ipv4" as const,
		home_location: "fsn1",
		description: "Test IP for Vitest",
	};
	...
	if (result.success) {
		...
		expect(result.response.floating_ip.home_location?.name).toBe(
...
test("update floating IP throws error if token is invalid", async () => {
	const result = await invalidHetzner.floatingIps.update(1, {
		name: "new-name",
	});
	expect(result.success).toBe(false);
});
...
test("update floating IP updates an existing IP", async () => {
	...
	if (result.success) {
		...
		expect(result.response.floating_ip.description).toBe(
...
test("update floating IP throws error if floating IP does not exist", async () => {
	const result = await workingHetzner.floatingIps.update(999999999, {
		name: "new-name",
	});
	expect(result.success).toBe(false);
});
