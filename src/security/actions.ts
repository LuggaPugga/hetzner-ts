...
export class CertificateActions extends BaseAPI {
	...
	async getAll(
		...
		| { success: true; response: CertificateActionsResponse }
	...
	async get(
		...
		| { success: true; response: CertificateActionResponse }
		| { success: false; response: APIError }
	> {
		return this.request<CertificateActionResponse>(
			`/certificates/actions/${actionId}`,
		);
	}
	...
	> {
		return this.request<CertificateActionResponse>(
			`/certificates/${certificateId}/actions/retry`,
			{
				method: "POST",
			},
		);
