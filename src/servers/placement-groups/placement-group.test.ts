		...
		if (listResult.success) {
			...
			expect(
				listResult.response.placement_groups.find(
					(placementGroup) =>
						placementGroup.id === result.response.placement_group.id,
				),
			).toBeDefined();
		}
	}
...
test("update a placement group and verify its properties", async () => {
	const name = generateUniqueName("test-placement-group");
	const result = await workingHetzner.servers.placementGroups.update(
		placementGroupToDelete,
		{
			name,
		},
	);
...
test("delete a placement group", async () => {
	const deleteResult = await workingHetzner.servers.placementGroups.delete(
		placementGroupToDelete,
	);
