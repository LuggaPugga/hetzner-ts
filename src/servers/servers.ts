...
export class Servers extends BaseAPI {
	...
	async getAll(
		...
		| { success: true; response: ServersResponse }
	...
	> {
		...
		return this.request<ServersResponse>(
			`/servers${queryString ? `?${queryString}` : ""}`,
		);
	}
	...
	async get(
		...
		| { success: true; response: { server: Server } }
		| { success: false; response: APIError }
	> {
	...
	getMetrics(
		...
		| { success: true; response: { metrics: Metrics } }
		| { success: false; response: APIError }
	> {
