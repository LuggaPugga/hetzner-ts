...
test("list server types throws error if token is invalid", async () => {
  const result = await invalidHetzner.servers.serverTypes.getAll()
  expect(result.success).toBe(false)
})

test("list server types", async () => {
  const result = await workingHetzner.servers.serverTypes.getAll()
  console.log(result)
  ...
  if (result.success) {
    ...
    const listResult = await workingHetzner.servers.serverTypes.getAll()
