...
test("test attach to placement group", async () => {
  ...
  if (server.success && placementGroup.success) {
    const result = await workingHetzner.servers.actions.addServerToPlacementGroup(
    ...
    const result2 = await workingHetzner.servers.actions.removeFromPlacementGroup(
      server.response.server.id,
    )
    ...
    await workingHetzner.servers.placementGroups.delete(placementGroup.response.placement_group.id)
  }
})
