...
test("create firewall successfully", async () => {
	...
	if (result.success) {
		...
		const deleteResult = await workingHetzner.firewalls.delete(
			result.response.firewall.id,
		);
		expect(deleteResult.success).toBe(true);
	}
});
...
test("create firewall throws error if token is invalid", async () => {
	const result = await invalidHetzner.firewalls.create({
...
test("update firewall successfully", async () => {
	...
	const updateResult = await workingHetzner.firewalls.update(firewallId, {
		name: updatedName,
	});
...
test("update firewall throws error if token is invalid", async () => {
	const result = await invalidHetzner.firewalls.update(123, {
...
test("update firewall throws error if firewall does not exist", async () => {
	const result = await workingHetzner.firewalls.update(999999999, {
		name: "test-update-nonexistent",
	});
	expect(result.success).toBe(false);
});
